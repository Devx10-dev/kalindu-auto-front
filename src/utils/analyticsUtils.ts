import { DailySummery, MonthlySummary, TotalSummary, WeeklySummary, YearlySummary } from "@/types/salesAndExpenses/saleAndExpenseTypes";
...
function processWeeklyData(dailySummaries: DailySummery[]): WeeklySummary[] {
  ...
  dailySummaries.forEach(summary => {
    const date = new Date(parseInt(summary.date[0]), parseInt(summary.date[1]) - 1, parseInt(summary.date[2]));
    ...
    weekSummary.startDate = new Date(Math.min(weekSummary.startDate.getTime(), date.getTime()));
    weekSummary.endDate = new Date(Math.max(weekSummary.endDate.getTime(), date.getTime()));
  });
...
function calculateTotalSummary(dailySummaries: DailySummery[]): TotalSummary {
    ...
    let startDate = new Date(parseInt(dailySummaries[0].date[0]), parseInt(dailySummaries[0].date[1]) - 1, parseInt(dailySummaries[0].date[2]));
    ...
    dailySummaries.forEach(summary => {
      const currentDate = new Date(parseInt(summary.date[0]), parseInt(summary.date[1]) - 1, parseInt(summary.date[2]));
