...
const MultiSelectPriceInput: React.FC<MultiSelectPriceInputProps> = ({
  ...
  const fillRemainingAmount = (id: string) => {
    const currentTotal = Object.values(selectedOptions).reduce(
      (sum, value) => sum + (value || 0),
      0,
    );
          ...
          <Popover key={option.id}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                <span className={`${textSizes[size]} font-medium text-center`}>
                  {option.label}
                </span>
                {option.id in selectedOptions && !option.disabled && (
                  <Check
                    className={`absolute top-1 right-1 ${iconSizes[size] === "w-12 h-12" ? "w-6 h-6" : "w-4 h-4"} text-green-500`}
                  />
                )}
                {option.disabled && option.disabledReason && (
                  <HelpCircle
                    className={`absolute top-1 right-1 ${iconSizes[size] === "w-12 h-12" ? "w-6 h-6" : "w-4 h-4"} text-gray-400`}
                  />
                )}
              </button>
            </PopoverTrigger>
        ...
        {Object.entries(selectedOptions).map(([id, value]) => {
          ...
          return (
            <div key={id} className="flex items-center gap-2">
              <Label htmlFor={`price-${id}`} className="w-32 flex-shrink-0">
                {option.label}
              </Label>
              <Input
                id={`price-${id}`}
                ...
                className="w-32"
              />
              <Button onClick={() => fillFullAmount(id)} size="sm">
                Full
              </Button>
              <Button
                onClick={() => fillRemainingAmount(id)}
                size="sm"
                variant="secondary"
              >
                Remaining
