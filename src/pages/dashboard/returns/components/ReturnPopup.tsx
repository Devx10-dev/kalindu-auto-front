...
import { CreditCard, Coins, CheckCircle, Smartphone, Laptop, Tablet, Monitor, Landmark, Banknote, Save } from 'lucide-react';
...
interface ReturnPopupProps {
  isOpen: boolean;
  onClose: () => void;
  onProceed: (cashbackType: string, paymentDetails: { [key: string]: number | null }) => void;
  invoiceType: string;
...
const ReturnPopup: React.FC<ReturnPopupProps> = ({ 
  ...
  const [paymentDetails, setPaymentDetails] = useState<{ [key: string]: number | null }>({});
  const [createResponse, setCreateResponse] = useState<InvoiceState | null>(null);
  ...
  const handleProceed = () => {
    if ((returnType === 'CASHBACK' || returnType === 'PARTIAL') && !cashbackType) {
    //   alert('Please select a cashback option');
      toast({
        title: "Invalid return details",
        description: "Please select a cashback option! This will determine how the return amount will be processed.",
        variant: "destructive",
      });
      return;
    }
    ...
    //   alert('Please enter the return amount');
      toast({
        title: "Invalid return details",
        description: "Please enter the return amount! This will determine the amount to be returned to the customer.",
        variant: "destructive",
      });
      return;
    }
    if (returnType === 'EXCHANGE' && !newInvoiceType) {
      ...
      toast({
        title: "Invalid return details",
        description: "Please select a new invoice type! This will determine how the return amount will be processed.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handlePaymentProceed = () => {
    const totalPayment = Object.values(paymentDetails).reduce((sum, value) => sum + (value || 0), 0);
    if (totalPayment !== paymentDifference) {
    //   alert('The total payment must equal the payment difference');
      toast({
        title: "Invalid payment details",
        description: "The total payment must equal the payment difference! Please adjust the payment amounts.",
        variant: "destructive",
      });
      return;
    }
  ...
  const paymentOptions = [
    ...
    { id: 'CREDIT', icon: CreditCard, label: 'Credit', disabled: newInvoiceType === 'CASH' },
  ];
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const createdInvoice = await createReturnMutation.mutateAsync(requestData);
  ...
  useEffect(() => {
    ...
    const handlePrint = () => {
      if(createResponse){
        if(createResponse.newInvoiceType === 'CASH'){
          navigate(`/dashboard/invoice/cash/${createResponse.newInvoiceId}`);
        }else if(createResponse.newInvoiceType === 'CRE'){
          navigate(`/dashboard/invoice/creditor/${createResponse.newInvoiceId}`);
        }
      }
    }
    ...
    useEffect(() => {
      if(returnType === 'PARTIAL' || returnType === 'EXCHANGE'){
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose} modal={true} >
      <DialogContent className='sm:max-w-[800px] max-w-[95vw] max-h-[90vh] overflow-y-auto py-0'  onInteractOutside={(e) => {e.preventDefault();}}>
        ...
        <div className="grid p-4 border border-muted-gray bg-muted-background rounded-md w-full">
            <div className="grid grid-cols-3 gap-2 w-full h-full">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <AmountCard
                        amount={remainingDue ? parseFloat(remainingDue.toFixed(2)) : 0}
                        color="#FFAAAA"
                        fontStyle="font-semibold"
                        withoutCurrency={false}
                    />
          ...
          <>
              ...
              <div className='flex flex-col gap-2 justify-between items-right'>
                ...
                <p className='text-sm text-muted-foreground'>
                  ...
                  {cashbackType === 'ACCOUNT-CREDIT' && (
                    <span>
                      Info: Refund the return amount to the customer's credit account.
                    </span>
                  )}
                </p>
              </div>
            ...
            {(returnType === 'EXCHANGE' || returnType === 'PARTIAL') && (
              <div className='flex flex-col gap-2 justify-between items-right'>
                <div className='flex flex-col gap-2'>
                  ...
                  <p className='text-md text-muted-foreground'>
                    Please select a new invoice type for the customer. This will determine how the exchange items will be processed.
                  </p>
                </div>
                <InlineIconRadioGroup
            ...
            {returnType === 'BALANCED' && (
                <div className='flex flex-col gap-2 justify-between items-right'>
                    ...
                    <p className='text-md text-muted-foreground'>
                        The return amount is balanced with the remaining due amount. No additional payment required and no cashback is done.
                        </p>
                    </div>
                </div>
                )}      
          ...
          <>
            <div className="py-4">
              ...
              <div className="mt-4 p-4 border border-muted-gray bg-muted-background rounded-md">
                ...
                {returnType === 'CASHBACK' && (
                  <p><strong>Cashback Method:</strong> {cashbackType === 'CASHBACK' ? 'Cash Out' : 'Refund to Credit Account'}</p>
                )}
                {returnType === 'PARTIAL' && (
                  <>
                    <p><strong>Partial Return Amount:</strong> Rs. {returnAmount.toFixed(2)}</p>
                    <p><strong>Cashback Method:</strong> {cashbackType === 'CASHBACK' ? 'Cash Out' : 'Refund to Credit Account'}</p>
                  </>
                )}
                {returnType === 'EXCHANGE' && (
                  <>
                    <p><strong>New Invoice Type:</strong> {newInvoiceType === 'CASH' ? 'Cash Invoice' : 'Credit Invoice'}</p>
                    {paymentDifference > 0 && (
                      <>
                        <p><strong>Additional Payment Required:</strong> Rs. {paymentDifference.toFixed(2)}</p>
                        <p><strong>Payment Methods:</strong></p>
                        <ul className="list-disc list-inside">
                          {Object.entries(paymentDetails).map(([method, amount]) => (
                            <li key={method}>{method}: Rs. {amount?.toFixed(2)}</li>
                          ))}
                        </ul>
                      </>
                    )}
                ...
                {returnType === 'BALANCED' && (
                    <p>Return amount is balanced with the remaining due amount. No additional payment required and no cashback is done</p>
                )}
              </div>
              <p className="font-semibold mt-4">Are you sure you want to process this return?</p>
            </div>
            {/* <DialogFooter>
        ...
        {!showConfirmation && !showSuccess && !showPaymentStep && (
          <DialogFooter className='sticky bottom-0 z-100 bg-white py-5'>
            <div className='flex flex-row w-full justify-end gap-3'>
                <Button onClick={onClose} variant='destructive' className='bg-red-400'>Cancel</Button>
