        ...
        {options.map((option) => (
          <Popover key={option.id}>
            <PopoverTrigger asChild>
              ...
              >
                <div className={`flex flex-col items-center justify-center h-full ${option.disabled ? 'opacity-50' : ''}`}>
                  <option.icon className={`${iconSizes[size]} mb-2`} />
                  <span className={`text-sm font-medium text-center px-2 ${size === 'sm' ? 'text-xs' : ''}`}>
                    {option.label}
                  </span>
                </div>
                {selectedId === option.id && !option.disabled && (
                  <Check className={`absolute ${checkMarkSizes[size]} text-green-500`} />
                )}
                {option.disabled && option.disabledReason && (
                  <HelpCircle className={`absolute ${checkMarkSizes[size]} text-gray-400`} />
                )}
              </button>
            </PopoverTrigger>
            {option.disabled && option.disabledReason && (
