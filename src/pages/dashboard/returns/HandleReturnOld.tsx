...
function HandlingReturnOld() {
  const {
    ...
    setVatAmount,
    setVatPercentage,
    setReturnVatPercentage,
    setReturnVatAmount,
    setReturnDiscountAmount,
    setReturnDiscountPercentage,
    ...
    setDiscountForSelectedReturnItems,
    setVatForSelectedReturnItems,
  ...
  const getVatPercentage = (
    ...
    if (invoiceType === "CASH") {
      return vatAmount > 0 ? (vatAmount / totalPrice) * 100 : 0;
    } else if (invoiceType === "CRE") {
      return vatAmount > 0 ? (vatAmount / (totalPrice - vatAmount)) * 100 : 0;
    }
    return 0;
  };
  ...
  const getDiscountPercentage = (
    ...
    if (invoiceType === "CASH") {
      return discountAmount > 0 ? (discountAmount / totalPrice) * 100 : 0;
    } else if (invoiceType === "CRE") {
      return discountAmount > 0
        ? (discountAmount / (totalPrice - vatAmount + discountAmount)) * 100
        : 0;
    }
    return 0;
  };
  ...
  useEffect(() => {
    if (selectedInvoice) {
      ...
      setReturnVatPercentage(vatPercentage);
      setReturnVatAmount(selectedInvoice?.vat);
      ...
      setReturnDiscountPercentage(discountPercentage);
      setReturnDiscountAmount(selectedInvoice?.discount);
    }
  }, [selectedInvoice, setReturnVatAmount, setReturnVatPercentage]);

  useEffect(() => {
    console.log("Return Item Value: ", returnItemValue);
    setDiscountForSelectedReturnItems(
      returnItemValue
        ? parseFloat(
            (returnItemValue * (returnDiscountPercentage / 100)).toFixed(2),
          )
        : 0,
    );
    setVatForSelectedReturnItems(
      returnItemValue
        ? parseFloat(
            (
              (returnItemValue -
                returnItemValue * (returnDiscountPercentage / 100)) *
              (returnVatPercentage / 100)
            ).toFixed(2),
          )
        : 0,
    );
  }, [returnItemValue, returnDiscountPercentage, returnVatPercentage]);
