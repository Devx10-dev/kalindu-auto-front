...
import { CreditInvoiceStats, PieChartData } from "@/types/invoice/creditorInvoice"
...
export function CreditorStatPieChart() {

    const [chartData, setChartData] = React.useState<PieChartData[]>([
        { status: "overdue", creditors: 0, fill: "#FFD700" },
        { status: "due", creditors: 0, fill: "#FF4C61" },
  ...
  const {
        ...
        isLoading,
        error,
    } = useQuery<CreditInvoiceStats, Error>({
    ...
    React.useEffect(() => {
        if (data) {
            setChartData([
                { status: "overdue", creditors: data.overdueCreditors, fill: "#FFD700" },
                { status: "due", creditors: data.dueCreditors, fill: "#FF4C61" },
                { status: "other", creditors: data.totalCreditors - (data.dueCreditors + data.overdueCreditors), fill: "#69c893" },
              ])
        }
    }, [data]);
        ...
        <div className="grid grid-cols-2 grid-rows-2 p-4 gap-x-5 w-full">
            <div className="flex col-span-1 row-span-1 justify-between">
                <div>                
                    ...
                    {!isLoading ? data ? contentRender("currencyAmount",currencyAmountString(data.totalCreditBalance)) : contentRender("currencyAmount","Rs. 0.00") : <Skeleton className="w-40 h-8" />}
                </div>
            </div>
            <div className="flex col-span-1 row-span-1 justify-between">
                    ...
                    <TooltipProvider>
                        <Tooltip>
                            ...
                            <div className="flex gap-1 items-center"> 
                                ...
                                <Badge className="w-fit" variant="secondary" >{!isLoading ? data?.totalOverdueInvoices : (<Skeleton className="h-5 w-5"/>)}</Badge>
                            </div>
                            </TooltipTrigger>
                            <TooltipContent>
                    ...
                    <TooltipProvider>
                        <Tooltip>
                            ...
                            <div className="flex gap-1 items-center">
                                ...
                                <Badge className="w-fit" variant="secondary" >{!isLoading ? data?.totalDueInvoices : (<Skeleton className="h-5 w-5"/>)}</Badge>
                            </div>
                            </TooltipTrigger>
                            <TooltipContent>
            ...
            <div className="col-span-1 row-span-1">
                ...
                <h1 className="text-xl font-bold text-black">
                    {!isLoading ? data ? contentRender("currencyAmount",currencyAmountString(data.dueAmount)) : contentRender("currencyAmount","Rs. 0.00") : (<Skeleton className="w-40 h-8" />)}
                </h1>
            </div>
            <div className="col-span-1 row-span-1">
                <p className="text-sm text-muted-foreground">Overdue Amount</p>
                <h1 className="text-xl font-bold text-black">
                    {!isLoading ? data ? contentRender("currencyAmount",currencyAmountString(data.overdueAmount)) : contentRender("currencyAmount","Rs. 0.00") : (<Skeleton className="w-40 h-8" />)}
                </h1>
            </div>
        </div>
      </CardContent>
