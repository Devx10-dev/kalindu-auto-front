...
import { useCreditInvoiceListStore, useInvoiceListStore } from "../view-invoices/context/InvoiceListState";
...
function SingleInvoiceCredit() {
  ...
  }, [invoiceData, isAvailableInStore, invoiceLoading, addCreditInvoiceToStore]);
          ...
          <div className="gap-5" style={{ flex: 9 }}>
              ...
              <div className="grid grid-cols-3 gap-2 mb-5">
                <div className="flex flex-col gap-1">
                  ...
                  <div className="border border-muted-gray p-1 rounded-md mr-5">
                    <p className="text-lg ">{invoiceDetails?.creditor.contactPersonName}</p>
                  </div>
                </div>
                <div className="flex flex-col gap-1">
                  <p className="text-sm text-muted-foreground">Primary Contact</p>
                  <div className="border border-muted-gray p-1 rounded-md mr-5">
                    <p className="text-lg">{invoiceDetails?.creditor.primaryContact}</p>
                  </div>
                </div>
                <div className="flex flex-col gap-1">
                  <p className="text-sm">Secondary Contact</p>
                  <div className="border border-muted-gray p-1 rounded-md mr-5">
                    <p className="text-lg">{invoiceDetails?.creditor.secondaryContact}</p>
                  </div>
                </div>
              </div>
                ...
                <Accordion  type="single" collapsible className="w-full" >
                  <AccordionItem value="1">
                    <CardHeader className="flex-row justify-between items-center">
                      <div>
                      <CardTitle className="text-md">Outsourced Items</CardTitle>
                      <CardDescription className="text-sm">
                        This section contains all the outsourced items
                      </CardDescription>
                      </div>
                      <div className="flex items-center gap-2">
                        { invoiceDetails?.invoiceItems.filter((item) => item.outsourced).length === 0 && ( <Badge variant="secondary" className="w-fit p-2 w-full border-radius-5 ">No Items</Badge>)}
            ...
            <div>
              <Card className="mt-4 ">
                ...
                <CardHeader className="flex-row justify-between items-center">
                  <div>
                  <CardTitle className="text-md">CommissionDetails</CardTitle>
                  <CardDescription className="text-sm">
                    This section contains all the commission details
                  </CardDescription>
                  </div>
                  <div className="flex items-center gap-2">
                    { invoiceDetails?.commissions.length === 0 && ( <Badge variant="secondary" className="w-fit p-2 w-full border-radius-5 ">No Commissions</Badge>)}
                    <AccordionTrigger className="p-2 rounded-md bg-primary-500 border border-primary-500"/>
                  </div>
                </CardHeader>
                <AccordionContent>
