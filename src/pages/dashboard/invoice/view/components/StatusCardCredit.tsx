...
function StatusCardCredit({
  ...
  const [statusIcon, setStatusIcon] = useState(<CheckCircle2 size={20} color="green" />);
  ...
  const calculateProgress = (dueTime: number[],issuedTime: number[], maxDueWeeks: number) => {
    const dueDate = new Date(dueTime[0], dueTime[1], dueTime[2]);
        ...
        <div className="flex items-center">
          <div className="w-full">
            <div className="flex gap-2">
              <div className={cn("flex items-center justify-center rounded-full bg-green",iconWrapperClassName)}>
                {statusIcon}
              </div>
            ...
            <div className="flex-col items-center mt-2">
              <Progress value={60} className="h-2 color-green" indicatorClassName="bg-yellow-500" />
              <p className="text-xs font-regular text-gray-500 mt-1 text">5 days remaining</p>
            </div>
          </div>
        </div>
      </Card>
