...
class CashInvoiceService extends Service {
    ...
    async createCashInvoice(cashInvoiceData: InvoiceState): Promise<InvoiceState> {
        try {
            const response = await this.api.post<InvoiceState>(CASH_INVOICE_URL, cashInvoiceData);
